    ...
    <header class="bg-blanco-puro/80 backdrop-blur-md shadow-md border-b border-bordes sticky top-0 z-50">
      <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
          ...
          <h1 class="text-xl font-bold bg-gradient-to-r from-amber-600 to-amber-800 bg-clip-text text-transparent">
            Ahorcado
          </h1>

          <div v-if="isAuthenticated" class="flex items-center space-x-2">
            <div class="flex items-center space-x-2 px-3 py-1 rounded-lg bg-amber-50">
              <span class="text-2xl">🎫</span>
              <div>
                <div class="text-xs text-texto-secundario">Tickets</div>
                <div class="font-bold text-amber-600">{{ ticketsDisponibles }}</div>
              </div>
            </div>
          </div>
          <div v-else>
...
import HangmanGame from '@/components/games/HangmanGame.vue'
...
const iniciarJuego = async () => {
  ...
  const exito = await hangmanStore.iniciarJuego(user.value.id)

  if (!exito && hangmanStore.yaJugoHoy) {
